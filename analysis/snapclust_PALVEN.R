...
# load in SNP data
d = read.snp("~/Dropbox/Encelia/analysis/hybrid_zones/input_files/PALVEN.miss0.8.snp")
...
tbl$sample = gsub("-", "", inds$V1)

res = inner_join(res, tbl, by = c("ind" = "sample"))
saveRDS(res, "~/Desktop/PALVEN_hybclass")
...
saveRDS(df, "~/Desktop/PALVEN_sim_hybclass")
